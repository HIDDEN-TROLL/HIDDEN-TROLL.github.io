        ...
        <tr v-for="(l,i) in yList" :key="i + 'a'"> <!-- 这里遍历外层数组，获取yList项-->
          ...
          <!-- 这里遍历属yList每一项添加首列控制类，v-if控制列表显示内容-->
            ...
            <template v-if="yKey==='statis_date'">
              <router-link :to="{name:'AttendanceSheetStudentDetail',query:{id:yList[i][yKey].id,name:yList[i][yKey].sName}}">
            ...
            <template v-else>
              ...
              <span style="display:inline-block;padding: 0px 5px;" v-if="typeObj.type==2 || typeObj.type==0">
              ...
              <span style="display:inline-block;padding: 0px 5px;" v-if="typeObj.type==3 || typeObj.type==0">
