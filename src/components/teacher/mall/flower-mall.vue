  ...
  <div id="flower-mall">
    ...
    <div class="go-back">
      <van-nav-bar :fixed="true" title="红花商城" left-text="返回" left-arrow @click-left="onClickLeft">
        <span slot="right" class="record right-text" @click="exchange">
          <span class="title">红花兑换</span>
         <van-icon name="arrow"  title="红花兑换"/>
       </span>
      </van-nav-bar>
    </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      exchange(){
        this.$router.push({name:"Exchangeprizes"})
      },
      selectFlowerMall(){
          ...
          this.selectFlowerMall().then(response=>{
            if (Array.isArray(response)){
              let res = response.map(item=>{
                item.imgurl = typeof item.imgurl === 'string' ? JSON.parse(item.imgurl) : item.imgurl;
                return item;
              });
              if (res.length<=8){
                this.loading = false;
