<template>
  <div class="teacher_homework_manage_student_paginate">
    <p class="top">
      ...
      <div class="page" v-for="(page, index1) in renderArray" :key="index1">
        <span class="item" v-for="(item, index2) in page" :key="index2">
          <button
            ...
            <span v-if="item.workInfo.isSubmit" class="badge"></span>
          </button>
        </span>
      </div>
    </div>
...
export default {
  name: 'teacher_homework_manage_student_paginate',
  props: {
    index: {
      type: Number,
      default: 0,
  ...
  methods: {
    ...
    init(focus = false) {
      if (!focus && (!this.show || this.isInit)) {
        return
      }
      this.isInit = true
...
.teacher_homework_manage_student_paginate {
  /*height: 100%;*/
  overflow: hidden;
  display: flex;
