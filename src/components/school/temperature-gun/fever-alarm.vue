<template>
    ...
    <div class="body">
        ...
        <div class="top_mes">
            <van-row>
                <van-col span="8" style="text-align: left">共{{test.totalCount}}个学生</van-col>
                <van-col span="8" style="text-align: center;color: red;font-weight: 700">{{test.feverNumber}}人发烧</van-col>
                <van-col span="8" style="text-align: right;color: red;font-weight: 700">{{test.feverCount}}人发烧</van-col>
            </van-row>
        </div>
        ...
        <ChangeDate ref="child" @change="changeFun($event)"> </ChangeDate>
...
<script>
    ...
    import { NavBar,Switch,Tab,Tabs,Row, Col,Dialog,Button,Toast,Icon,Cell,DatetimePicker,Popup,Actionsheet,List    } from 'vant';
    ...
    export default {
        ...
        methods:{
            changeFun(data){
                ...
                this.changeStart = data.startDate.toLocaleDateString().replace(/\//g, '-')
                this.changeEnd = data.endDate.toLocaleDateString().replace(/\//g, '-')
            ...
            jump(stuId){
                this.$router.push({ name:'PersonalTemperature' ,query:{stuId:stuId} })
            },
            //年级选择点击事件
            onGradeActionsheet() {
                this.showGrade = true;
            ...
            onClickRight(){
                console.log(1)
                this.$router.push({name:'StuTemperature'})
