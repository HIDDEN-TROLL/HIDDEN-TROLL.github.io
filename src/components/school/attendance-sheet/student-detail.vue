<template>
  ...
  <div id="student-detail">
    ...
    <div class="content">
      <div class="sign-tab">
        <van-tabs type="card" @change="tabType">
          <van-tab title="进园签到">
            <div class="sign-info">
              <div class="flex-box">
                <div class="sub">本月总进园：{{tMonth ? tMonth : 0}}次</div>
                <div class="sub" style="text-align: center">本周总进园：{{tWeek ? tWeek: 0}}次</div>
                <div class="sub" style="text-align: right">今日总进园：{{tDay ? tDay: 0}}次</div>
              </div>
            </div>
            <div class="total">
              <span class="num">共{{ recordNumber }}条数据</span>
            </div>
            <div class="list">
              <van-list
                v-model="loading"
          ...
          <van-tab title="离园签退">
            <div class="sign-info">
              <div class="flex-box">
                <div class="sub">本月总离园：{{tMonth ? tMonth : 0}}次</div>
                <div class="sub" style="text-align: center">本周总离园：{{tWeek ? tWeek: 0}}次</div>
                <div class="sub" style="text-align: right">今日总离园：{{tDay ? tDay: 0}}次</div>
              </div>
            </div>
            <div class="total">
              <span class="num">共{{ recordNumber }}条数据</span>
          ...
          <van-tab title="考勤记录">
            <div class="sign-info">
              <div class="flex-box">
                <div class="sub" style="text-align: center">本月考勤记录：{{tMonth ? tMonth : 0}}次</div>
                <!--<div class="sub" style="text-align: center">本周总离园：{{tWeek ? tWeek: 0}}次</div>-->
                <div class="sub" style="text-align: center">今日考勤记录：{{tDay ? tDay: 0}}次</div>
              </div>
            </div>
            <div class="total">
              <span class="num">共{{ recordNumber }}条数据</span>
...
<script>
  ...
  import { getStudentAttendance } from '@/api/school/attendance-sheet';
  ...
  export default {
    ...
    data(){
      return {
        ...
        loading: false,
        finished: false,
        studentId: this.$route.query.id,//学生id
        type:1,//1进园 2 离园
        time:0,//默认查询所有
    ...
    methods: {
      ...
      selectList(){
        ...
        if (this.isFromTemplate) {
          ...
          if (this.$route.query.templateId && parseInt(this.$route.query.templateId) > 0) {
            this.queryData.template_id = this.$route.query.templateId
          } else if (this.$route.query.template_id && parseInt(this.$route.query.template_id) > 0) {
            this.queryData.template_id = this.$route.query.template_id
          }  else if (this.templateId && parseInt(this.templateId) > 0) {
            this.queryData.template_id = this.templateId
          }
        }

        getStudentAttendance(this.queryData)
                .then(res=>{
                  if (this.lof === 1){
                    toast.clear();
                    this.lof = null;
              ...
              this.recordNumber = res.data.data.paginate ? res.data.data.paginate.total : 0
                    if (this.type === 1){
