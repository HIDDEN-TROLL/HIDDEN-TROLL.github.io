<template>
  <div class="content">
    ...
    <div class="bar_mas">
      ...
      <div class="info">
        ...
        <div class="mas_name_div">
          ...
          <a href="javascript:;" class="mas_name_b">
            负责班级：{{ chargeClassesName }}
            <br>{{accountInfo.schoolName}}
          </a>
          <p></p>
        </div>
    ...
    <p v-if="isShowStopService" class="stop_service">{{ data.other.stopService.text }}</p>
...
import { getMyPageData, updateProfile } from '@/api/teacher/home'
...
export default {
  ...
  computed: {
    ...
    chargeClassesName() {
      const chargeClasses = this.accountInfo.chargeClasses
      if (Array.isArray(chargeClasses) && chargeClasses.length > 0) {
        return chargeClasses.join('、')
      }
      return '无'
    },
  },
  ...
  async created() {
    ...
    const data = await this.getPageExtraData('TeacherHomeMy')
    if (data !== null) {
      this.data = data
      return
    }
