      ...
      <div class="upload-video">
        ...
        <div class="video-box">
          ...
          <div class="upload" :class="videoArr.length>0?'curr':''" v-if="videoArr.length===0">
            <van-uploader :after-read="onReadVideo" multiple accept="video/mp4, video/webm">
              <van-icon name="plus"/>
            </van-uploader>
          </div>
        </div>
...
<script>
  ...
  import { basicInfoConfig, postCommunity } from '@/api/school/community'
    ...
    methods: {
      ...
      infoData() {
        ...
        basicInfoConfig()
                .then(res => {
                  ...
                  this.$loadingHide()
                  if (res.status === 200) {
                    if (res.data.data.checkSwitch === '0') {
                      this.btnText[0].title = '发布动态'
                    } else if (res.data.data.checkSwitch === '1') {
                      this.btnText[0].title = '提交校长审核'
                    }
                    this.permissions = res.data.data.checkSwitch;
      ...
      turnBase64(dataURI) {
        try{
          ...
          // console.log("这里",mimeString,byteString,arrayBuffer,intArray)
          for (let i = 0; i < byteString.length; i++) {
            intArray[i] = byteString.charCodeAt(i);
          }
          return new Blob([intArray], { type: mimeString });
        }catch (e) {
          console.error('这里出错:', e);
        }
      ...
      uploadFile(fileType, fileUrl) {
        ...
        getUploadOssUrl(fileType, undefined, 1)
                .then(res => {
                  ...
                  //console.log(res);
                  if (res.data.meta && res.data.meta.code === 0) {
                    this.axios.put(res.data.data[0].url, this.turnBase64(fileUrl) , {
                      headers: { 'Content-Type': res.data.data[0].type },
                      // withCredentials:true,
                      onUploadProgress: progressEvent => {
                        var complete = (progressEvent.loaded / progressEvent.total * 100 | 0);
                      ...
                      console.log("上传参数",fileUrl,this.turnBase64(fileUrl),res.data.data[0].type)
                      this.$loadingHide();
                      if (response.status === 200) {
                        Toast.success({
                          forbidClick: true, // 禁用背景点击
                      ...
                      // alert(this.vedioKey +"隔开"+ this.videoArr[0].src)
