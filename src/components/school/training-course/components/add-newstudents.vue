<template>
    <div class="detail-box">
        <div class="go-back">
            <van-nav-bar title="新学生入班" left-text="返回" left-arrow @click-left="back()"/>
        </div>
        <div class="tab">
...
<script>
    ...
    import { NewTrainingStudents } from '@/api/school/training-course'
    // 获取所有学生
    import { getUnallocatedStu } from '@/api/school/training-course'
    ...
    export default {
        ...
        data() {
            return {
                ...
                gradeList: ['全部年级', '一年级','二年级','三年级','四年级','五年级','六年级','初中','未设置'],
                gradeIndex: 0,
        ...
        mounted(){
            this.id === undefined ? this.$router.replace({name:'trainingCourse'}) : null
        ...
        methods: {
            ...
            onActionSheetSelect(item) {
                ...
                if(item.index === 0){
                    this.stuNumber = this.student.length
                }else{
                    this.stuNumber = 0
                    for(let i = 0;i<this.student.length;i++){
                        this.student[i].gradeName == this.ChiocegradeName ? this.stuNumber += 1 : null
                    }
                }
            },
            operateBtn() {
            ...
            //获取未分班的学生列表
            selectStudent() {
                getUnallocatedStu(this.id)
                    .then(res => {
                        if (res.data.meta && res.data.meta.code === 0) {
                            // if (res.data.data.length > 0) {
                            this.student = res.data.data
                            this.stuNumber = res.data.data.length
                            console.log(this.IdList)
                            console.log(this.student)

                            for(let i=0;i<this.IdList.length;i++){
                                for( let a = 0;a<this.student.length;a++){
                                    if(this.student[a].id === this.IdList[i]){
                    ...
                    throw new Error("培训班获取未分班的学生列表异常！（无学生id）")
                })
            },
            //分配学生
            distributionStudent(stu) {
                stu= stu.concat (this.IdList)
                const data = {}
                if(stu){
                    data.students = stu
                }
                NewTrainingStudents(this.id, data)
                    .then(res => {
                        if (res.data.meta && res.data.meta.code === 0) {
                            Toast.success({
