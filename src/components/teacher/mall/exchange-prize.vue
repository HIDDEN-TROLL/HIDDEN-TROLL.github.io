        ...
        >
          <van-row gutter="16">
            <van-col span="12" v-for="(item,index) in prize" :key="index">
              <div class="prize-info"  v-if="item.stock>=1" @click="choose(index,item.active, item.worth,item.stock,{mallId:item.mallId,stock:item.stock,worth:item.worth})">
                ...
                <span class="title info" :class="{suff:number<item.worth || item.stock<1}">【{{item.name}}】</span>
                <span class="conditions info" :class="{suff:number<item.worth || item.stock<1}">需要：{{item.worth}} 朵红花</span>

                <span class="inventory info" :class="{suff:number<item.worth || item.stock<1}" >库存：{{item.stock}}/{{item.number}}</span>
                <span class="viewRecord info insufficient" v-if="item.stock<1">商品不足</span>

                <span class="viewRecord info" v-if="number>item.worth && item.stock>0">可兑换</span>
                <span class="viewRecord info insufficient" v-if="number<item.worth">红花不够</span>
              </div>

              <div class="prize-info" v-if="item.stock<1" >
                <div class="choosing" v-show="item.active">
                  <span class="icon-choose">
                    <van-icon name="passed" />
                  </span>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      operateBtn(){
        if( this.stop === true && this.mallInfo!=''){
          this.$loading();
          ...
          .then(res=>{
            ...
            throw new Error("兑换礼品异常！")
          })
          this.stop = false
        }

      },
      choose(index, active, num, stock, obj){
